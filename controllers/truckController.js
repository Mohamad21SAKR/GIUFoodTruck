...
/**
 * ✅ ADDED:
 * normalize statuses + accept typos
 */
...
async function getAllTrucks(req, res) {
  try {
    /**
     * ✅ UPDATED:
     * if DB has "avaliable" typo, this endpoint would hide trucks.
     * We fetch both.
     */
    const trucks = await db.select('*')
      .from('FoodTruck.Trucks')
      .where(builder => {
        builder.where('truckStatus', 'available').orWhere('truckStatus', 'avaliable');
      })
...
async function createTruck(req, res) {
  try {
    ...
    /**
     * ✅ UPDATED:
     * Normalize incoming statuses so you don’t end up mixing available/avaliable
     * BUT still accept input values.
     */
    const incomingTruckStatus = truckStatus ? normalizeStatus(truckStatus) : null;
    const incomingOrderStatus = orderStatus ? normalizeStatus(orderStatus) : null;

    const newTruck = {
      truckName,
      truckLogo: truckLogo || null,
      ownerId: user.userId,
      truckStatus: incomingTruckStatus ? (isAvailableStatus(incomingTruckStatus) ? 'available' : incomingTruckStatus) : 'available',
      orderStatus: incomingOrderStatus ? (isAvailableStatus(incomingOrderStatus) ? 'available' : incomingOrderStatus) : 'available'
    };
...
async function updateTruck(req, res) {
  try {
    ...
    /**
     * ✅ UPDATED:
     * Normalize status fields if provided (avaliable -> available)
     */
    if (truckStatus !== undefined) {
      const s = normalizeStatus(truckStatus);
      updateData.truckStatus = isAvailableStatus(s) ? 'available' : s;
    }
    if (orderStatus !== undefined) {
      const s = normalizeStatus(orderStatus);
      updateData.orderStatus = isAvailableStatus(s) ? 'available' : s;
    }
